...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        if connection_success:
            ...
            logger.warning("Database connection test failed - API will start without database connectivity")

        yield

    except Exception as e:
        logger.error(f"Failed to start application: {e}")
        # Don't raise - allow app to start even if there are startup issues
        logger.info("Starting API in degraded mode")
        yield
    finally:
